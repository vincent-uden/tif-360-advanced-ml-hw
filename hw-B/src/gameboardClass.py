...
class TGameBoard:
    def __init__(self, N_row, N_col, tile_size, max_tile_count, agent, stochastic_prob):
        ...
        if self.tile_size == 2:
            ...
            #   x x
            #   x x
            # Tile structure of tiles[i,j]: The first dimension denotes x
            # value, the length denotes the number of columns taken by the
            # tile. The second dimension consist of pairs giving the y range:
            # first element in the pair is the first row of the tile and the
            # second element second is the last row plus 1 of the tile for the
            # current column
    ...
    """Execute a move onto the gameboard

    Arguments:
    new_tile_x           -- the column of the tile (0 <= new_tile_x <= self.N_col)
    new_tile_orientation -- the number of 90 degree rotations of the tile
    """
    ...
    def fn_drop(self):
        ...
        for xLoop in range(len(curtile)):
            if self.tile_y + curtile[xLoop][1] > self.N_row:
                ...
                self.board[
                    self.tile_y + curtile[xLoop][0] : self.tile_y + curtile[xLoop][1],
                    (xLoop + self.tile_x) % self.N_col,
                ] = 1
